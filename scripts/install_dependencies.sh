...
if [ "$BOOST_VARIANT" == "release" ]; then
  if [ "$LINK" == "static" ]; then
    SENTRY_BUILD_ARGS="-DCMAKE_BUILD_TYPE=Release -DSENTRY_BUILD_SHARED_LIBS=OFF -DCURL_USE_STATIC_LIBS=ON"
  else
    SENTRY_BUILD_ARGS="-DCMAKE_BUILD_TYPE=Release -DSENTRY_BUILD_SHARED_LIBS=ON -DCURL_USE_STATIC_LIBS=OFF"
  fi
else
  if [ "$LINK" == "static" ]; then
    SENTRY_BUILD_ARGS="-DCMAKE_BUILD_TYPE=Debug -DSENTRY_BUILD_SHARED_LIBS=OFF -DCURL_USE_STATIC_LIBS=ON"
  else
    SENTRY_BUILD_ARGS="-DCMAKE_BUILD_TYPE=Debug -DSENTRY_BUILD_SHARED_LIBS=ON -DCURL_USE_STATIC_LIBS=OFF"
  fi
fi
